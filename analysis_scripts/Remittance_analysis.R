...
# Remittance data
rmt <- fd %>% 
  select(village, hhid, name, week_num, week, date, 
...
rmt_method_summary <- rmt_nonZero %>% 
  summarise("bank total" = sum(rmt_method_bank), "in person" = sum(rmt_method_person), 
            "mobile total" = sum(rmt_method_mobile), "money order" = sum(rmt_method_moneyorder),
            "other total" = sum(rmt_method_oth))

# Histogram
method_counts <- c(397, 472, 1, 1, 13)
Methods <- c("Bank", "In person", "Mobile", "Money Order", "Other")
method_dat <- data.frame(methods, method_counts, stringsAsFactors = T)

ggplot(method_dat, aes( x= Methods, y = method_counts, fill = Methods)) +
  geom_col() +
  labs(x = "Method of Recieving Remittance", y = "Count") +
  theme_classic() +
  ggtitle("")
...
Purpose <-  c("Food/utility purchases", "Tuition", "Asset/Durable Purchases", "Medical Expenses", "Other", "No Reason")
purpose_count <- c(594, 37, 27, 93, 128, 43)
purpose_dat <- data.frame(Purpose, purpose_count, stringsAsFactors = T)

ggplot(purpose_dat, aes(x = Purpose, y = purpose_count, fill = Purpose)) + geom_col() +
  labs(x = "Purpose for Recieving Remittance", y = "Count") +
  theme_classic() +
  ggtitle("")+
  rotate_x_text(angle = 22, size = rel(0.8))
# Get total income by village for each week then compare across 49 week period
...
# Total remittance for each village each week
...
# Remittance income plot
Total_rmt_plot <- ggplot(rmt_data_total_weeks, aes(x = weeks_rep, y = total_rmt_per_week, color = Villages)) + 
  geom_line() +
  theme_classic() +
...
# Same thing but with the average remitance instead of total
...
# average remittance for each village per week
...
average_rmt_plot <- ggplot(rmt_data_mean_weeks, aes(x = weeks_rep, y = mean_rmt_per_week, color = Villages)) + 
  geom_line() +
  theme_classic() +
  labs(x = "Weeks", y = "Average Village Remittance Income") +
  ggtitle("Average Remittance Income Per Village Over 49 Weeks")
average_rmt_plot
